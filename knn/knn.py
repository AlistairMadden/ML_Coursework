...
for i in range(0, len(testing_attributes[:,0])) :
    ...
    # print("Test data example : {} : result = {} : expected = {}".format((i+1), inv_classes[(int(results[0]))], inv_classes[(int(testing_labels[i]))]));

    # record results as tp/tn/fp/fn

    # print(results[0])
    # print(testing_labels[i])
    # print((int(results[0]) == testing_labels[i]) == True)


    # if ((int(results[0]) == testing_labels[i]) == True) : tp+=1
    # elif (results[0] == testing_labels[i] == 0) : tn+=1
    # elif (results[0] != testing_labels[i]) :
    #     if ((results[0] == 1) and (testing_labels[i] == 0)) : fp+=1
    # elif ((results[0] == 0) and (testing_labels[i] == 1)) : fn+=1
